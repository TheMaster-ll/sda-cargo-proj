...
import {
  ...
  Accordion, AccordionSummary, AccordionDetails, Chip, CircularProgress, useTheme
} from '@mui/material';
import { Search, Phone, BookOpen, MessageSquare, ChevronDown, PlayCircle, Code, MapPin, Mail, Clock } from 'lucide-react';
...
=======
      {/* Help Sections as FAQ Accordions */}
      <Box sx={{ mb: 5 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Typography variant="h5" fontWeight={700}>Help Topics</Typography>
          <Chip label={`${filteredSections.length} of ${sections.length} topics`} size="small" variant="outlined" />
        </Box>
        ...
        {filteredSections.map((section, i) => (
          <Accordion
            ...
            expanded={expanded === i}
            onChange={() => setExpanded(expanded === i ? false : i)}
            sx={{ borderRadius: '8px !important', mb: 1, '&:before': { display: 'none' }, boxShadow: 'none', border: `1px solid ${isDark ? '#334155' : '#e2e8f0'}` }}
          >
            <AccordionSummary expandIcon={<ChevronDown size={20} />}>
              <Typography variant="subtitle2" fontWeight={600}>{section.title}</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.8 }}>{section.content}</Typography>
>>>>>>> 4335c1f75fd7cb0f58bf51fdf229c13149ebca98
            </AccordionDetails>
          </Accordion>
        ))}
...
=======
                variant="body2" component="a" href={r.href} target="_blank" rel="noopener noreferrer"
>>>>>>> 4335c1f75fd7cb0f58bf51fdf229c13149ebca98
